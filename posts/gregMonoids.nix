...
'' # A Brief Overview of a Monoid

A monoid consists of:

  1. A set. The set will be notated as `S`
  1. A binary operator that is closed''(sideNote "this means it maps 2 elements
  of the set to an element in the same set")''. The binary operator will be
  noted as `◊`.
  1. An element of the set called the identity. The identity element will be
  notated as `e`.

The following equations[^also known as mathematical laws] must hold for every
monoid:

1. `∀ x ∈ S. x ◊ e = e ◊ x` (Identity law)
1. `∀ {x, y, z} ∈ S. x ◊ (y ◊ z) = (x ◊ y) ◊ z` (Associativity law)

The Identity law states that the identity element has no "affect" on any
element in the set. Keep in mind that this means that `(a ◊ b) ◊ e = a ◊ b`
also holds''( sideNote "`x = a ◊ b` in the Identity law above" )''. This
implies that the identity element has no affect on the result of combining
elements of a monoid when using the binary operator.

Here are a few examples:

1. The cardinal numbers with addition and `0`.
1. The cardinal numbers with multiplication and `1`.
1. Strings with string concatenation and the empty string.
1. Paints with paint mixing and a transparent paint (like a finishing paint).

# Monoids In Programming

Monoids can be encoded in most programming languages''(sideNote "and typically
the ones that can't are those that don't allow functions which is extremely
rare. 99.9% of the time you should be able to encode monoids")''. To do this 
we need to encode the 3 things that make up a monoid:

1. The set for a monoid corresponds to a type in a programming language. 
1. The binary operator corresponds to a function taking two arguments and
returning a value. The arguments and return value must all be assigned the
monoid's type.
1. The identity element corresponds to a selected value of the monoid's type.
To repeat the laws above: this can't be any value but a value that upholds the
Idenity law.

Here's a monoid encoded in C++: 

''(code "C" ''

//The monoid's set/type: std::string
//The binary operator  : concat
//The identity element : ""

std::string concat(std::string a, std::string b) {
  //assume the details for string concatenation hold here.
  ...
}
'')''
